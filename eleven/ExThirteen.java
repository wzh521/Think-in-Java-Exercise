...
class Controller {
 ...
 private List<Event> eventList =
    ...
    public void run() {
        while(eventList.size() > 0) { Iterator<Event> it =
                new LinkedList<Event>(eventList).iterator();
            while(it.hasNext()) {
            Event e = it.next();
                if(e.ready()) {
                System.out.println(e); e.action();
                it.remove();
            }
        }
        }
...
class GreenhouseControls extends Controller { private boolean light = false;
    public class LightOn extends Event {
        public LightOn(long delayTime) { super(delayTime); } public void action() { light = true; }
        public String toString() { return "Light is on"; }
    }
    public class LightOff extends Event {
        public LightOff(long delayTime) { super(delayTime); } public void action() { light = false; }
        public String toString() { return "Light is off"; }
    }
    private boolean water = false; public class WaterOn extends Event {
        public WaterOn(long delayTime) { super(delayTime); } public void action() { water = true; }
    ...
    public class WaterOff extends Event {
        ...
        public WaterOff(long delayTime) { super(delayTime); } public void action() { water = false; }
        public String toString() {
    ...
    public class ThermostatNight extends Event { public ThermostatNight(long delayTime) {
        ...
        public void action() { thermostat = "Night"; } public String toString() {
    ...
    public class ThermostatDay extends Event { public ThermostatDay(long delayTime) {
        ...
        public void action() { thermostat = "Day"; } public String toString() {
...
public class ExThirteen {
    public static void main(String[] args) {
        GreenhouseControls gc = new GreenhouseControls(); gc.addEvent(gc.new Bell(900));
